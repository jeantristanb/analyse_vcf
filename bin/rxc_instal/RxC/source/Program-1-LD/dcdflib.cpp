// downloaded on 06/29/06
// from http://www.csit.fsu.edu/~burkardt/cpp_src/dcdflib/dcdflib.html
...
// DZ: changed the header name from dcdflib.H to dcdflib.h
# include "dcdflib.h"

// DZ moved 2 following declarations from dcdflib.h
...
//
    ...
    mu = (int)fabs(exparg(&K1)); //DZ: cast
    k = (int)exparg(&K2); //DZ: cast
    if(k < mu) mu = k;
    t = mu;
    d = exp(-t);
S10:
...
S20:
    r = (*b-1.0e0)**x/ *y-*a;
    if(r < 1.0e0) goto S50;
    t = nm1; k = (int)t; //DZ: was: k = t = nm1;
    if(r < t) k = (int)r; //DZ: cast
S30:
//
...
//
{
  static double coef[4][5] = { //DZ: put brackets around the 1st dim
