// Time-stamp: <2008-08-21 14:09:40 zaykind> 
// (written by Dmitri Zaykin)
// Reference:
// Zaykin, DV, Pudovkin AI, Weir BS 2008. 
// Correlation-based inference for linkage disequilibrium
// with multiple alleles. Genetics (in press).
...
typedef set <int, less<int> > Si;
typedef set <string, less<string> > Ss;
typedef vector <string> Vs;
typedef map <string, int, less<string> > Ms;

double DprIJ(int **q, int n);

void ReadArgs(int ac, const char** av, string& FileName,
	      string& MissVal, int& sim, double& PvalThresh,
	      unsigned long& seed);

// Uniform random numbers based on Marsaglia's generators posted to
// sci.stat.math (with my fixing of a bug in the cyclic table)
