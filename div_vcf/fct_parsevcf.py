...
def GetHeaderVcf(LireVCF,DicInfoPop,GetLigneVCF=False ) :
   '''
   to keep header vcf
   '''
    Balise=True
...
def GetGenoDiploide(LigneVCFSplit, ParcoursInd, PosAncetre=None) :
    Geno=[0]*3
    if PosAncetre==None :
...
## Permet pour une position donnee, a une pop donne de recuperer les genotypes
def GetGenoHaploide(LigneVCFSplit, ParcoursInd, PosAncetre=None) :
    Geno=[0]*3
    if PosAncetre==None :
        for Cmt in ParcoursInd :
            A1=ValueGeno(LigneVCFSplit[Cmt][0])
...
def ReadFile(FileVCF, IsGZ=False) :
    try :
       if  IsGZ :
         Read=gzip.open(FileVCF)
       else :
         Read=open(FileVCF)
    except :
