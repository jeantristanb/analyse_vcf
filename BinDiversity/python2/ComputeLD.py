...
def ComputeFGTDeuxPos(Snp1,Snp2, MinFreqBon=0.99) :
    ...
    while Cmt<NbIndI:
         ...
         Cmt+=1
    NbInd=float(Dist[1][1] + Dist[1][2] + Dist[2][1] +Dist[2][2])
    PBon=NbInd/NbIndI
    if PBon<MinFreqBon :
       return None
    ...
    if pa1==0 or pb1==0 or pa1==1 or pb1==1:
       return None
    if Dist[1][1]==0 or Dist[2][2]==0 or Dist[1][2]==0 or Dist[2][1]==0:
       return True
    return False
...
def ComputeLDDeuxPos(Snp1,Snp2, MinFreqBon=0) :
    NbIndI=float(len(Snp1))
    Cmt=0
    Dist=[[0,0,0],[0,0,0],[0,0,0]]
    while Cmt<NbIndI:
...
def ComputeLD1Segment(ListeSnp, MinFreq,MinFreqBon=0.99): 
    NbSnp=len(ListeSnp)
    ListeDprime=[]
    ListeRcarre=[]
    ListeDprimeMinFreq=[]
    ...
    for CmtSnp in range(0,len(ListeSnp)-1) : 
        for CmtSnp1 in range(CmtSnp+1,len(ListeSnp)) : 
            (DPrime, Rcar,pa1,pb1)=ComputeLDDeuxPos(ListeSnp[CmtSnp],ListeSnp[CmtSnp1], MinFreqBon)
            if Rcar!=None :
    ...
    if NbCmpMinFreq>0:
       ...
       MeanDprimeMinFreq="NA"
       MeanRcarreMinFreq="NA"
    NbSignif=0
...
def CompareLD2Segment(ListeSnp1, ListeSnp2, MinFreq, MinFreqBon) :
    ...
    ListeDprimeMinFreq=[]
    ListeRcarreMinFreq=[]
    if len(ListeSnp1) > 0 and len(ListeSnp2)>0:
       NbInd=len(ListeSnp1[0])
       for CmtSnp1 in range(0,NbSnp1):
           for CmtSnp2 in range(0,NbSnp2):
               (DPrime, Rcar,pa1,pb1)=ComputeLDDeuxPos(ListeSnp1[CmtSnp1],ListeSnp2[CmtSnp2], MinFreqBon)
               if Rcar !=None :
                 if pa1>MinFreq and pb1>MinFreq and (1-pa1)>MinFreq and (1-pb1)>MinFreq:
                    ListeDprimeMinFreq.append(DPrime)
                    ListeRcarreMinFreq.append(Rcar)
