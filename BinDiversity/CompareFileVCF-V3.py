...
def GetInfoVCF(Fichier, FichierStatInd,FichierStatLocus ,Balisebcftools=False,ListePosCom=[], Entete=None, SaveInfo=True, ListeIndSouhaite=None) :
    Lire=open(Fichier)
    if (SaveInfo) :
       Dico={}
       DicoLocus={}
    ...
    for Ligne in Lire :
       if Ligne[0]!='#':
          ...
          elif (len(ListePosCom)==0 or NomPos in ListePosCom) and TmpSplit[6]=='PASS'  :
              if SaveInfo:
          ...
          if Ligne[0:6]=="#CHROM":
             ...
             if ListeIndSouhaite==None :
                ListeIndSouhaite=[x.lower() for x in TempListeInd]
             PosSouhaite=[]
             Cmt=0
             for Ind in TempListeInd:
                 Ind=Ind.lower()
                 ListeInd.append(Ind.lower())
                 if Ind in ListeIndSouhaite :
                    if SaveInfo :
                       Dico[Ind]={}
                    DicoInfoInd[Ind.lower()]={'NbHet':0, 'NbHomAlt':0, 'NbNC':0, 'NbHomRef':0}
                    PosSouhaite.append(Cmt)
                 Cmt+=1
             if len(PosSouhaite)==0 :
                sys.exit("Nb Ind souhaite == 0") 
             else :
                print len(PosSouhaite)
    EcrireLocus.close()
...
def CmpIndLocus(Dico1, DicoPuce) :
    ...
    for Ind in ListeInd:
        for Locus in ListeLocus :
            if Locus not in Dico1['DataAll'][Ind] :
               ...
               if Locus not in DicoPuce['DataAll'][Ind]:
                  print "and in DicoPuce" 
            elif Locus not in DicoPuce['DataAll'][Ind] :
               print "Ind " + Ind + " Locus "+ Locus + " Not present in Puce"
            else :
               TmpStat=GetScoreLocus(Dico1['DataAll'][Ind][Locus],DicoPuce['DataAll'][Ind][Locus], DicoPuce['InfoSnp'][Locus]["Ref"], DicoPuce['InfoSnp'][Locus]["Alt"][0])
...
def PrintResInd(FileSortie, ResParInd, Entete, BaliseWiki=False):
    ...
    #Chaine=sepEnteteDebCol+"Locus"+sepEnteteCol+"AllNC"+sepEnteteCol+"Loc1NC"+sepEnteteCol+"LocPuceNC"+sepEnteteCol+"RefHomBon"+sepEnteteCol+"AltHomBon"+sepEnteteCol+"RefHomFaux"+sepEnteteCol+"AltHomFaux"+ sepEnteteCol+"DeuxHetBon"+sepEnteteCol+"DeuxHetFaux"+sepEnteteCol+"NGSHomRefPuceHet1Bon"+sepEnteteCol+"NGSHomAltPuceHet1Bon"+sepEnteteCol+"NGSHomPuceHet2Faux"+sepEnteteCol+"PuceHomRefNGSHet1Bon"+sepEnteteCol+"PuceHomAltNGSHet1Bon"+sepEnteteCol+"PuceHomNGSHet2Faux"+"\n"
...
def CompareVCFWithPuce(DataVCF, DicoSnpPuce):
    DicoModele={}
    Cmt=0
    for Key in DataVCF:
        Modele=Key
        ...
        ResultatAll=CmpIndLocus(DicoSnp, DicoSnpPuce)
        ...
        EnteteCmpInfLocus=["Locus","AllNC","Loc1NC","LocPuceNC","RefHomBon","AltHomBon","RefHomFaux","AltHomFaux","DeuxHetBon","DeuxHetFaux","NGSHomRefPuceHet1Bon","NGSHomAltPuceHet1Bon","NGSHomPuceHet2Faux","PuceHomRefNGSHet1Bon","PuceHomAltNGSHet1Bon","PuceHomNGSHet2Faux"]
...
def GetDataVCF(ListeFileVCF, BaliseBCF, Entete, SaveInfo, ListeIndSouhaite=None) :
    Cmt=0
    Dic={}
    for File in ListeFileVCF :
        print File
        Dic[File]=GetInfoVCF(File,File+".brut.ind.stat" ,File+".brut.locus.stat" , BaliseBCF[Cmt], Entete=Entete, SaveInfo=SaveInfo, ListeIndSouhaite=ListeIndSouhaite)
        Cmt+=1
    return Dic
...
def CmpDeuxVCF(VCF1, VCF2, File, Nom1, Nom2, BaliseNew):
    ListeLocus=VCF1['InfoSnp'].keys()
    ListeLocus.extend(VCF2['InfoSnp'].keys())
    ListeLocus=set(ListeLocus)
...
#def NbDifference(A1,B1,A2,B2):
    ...
    if BaliseNew==True :
       ...
       Ecrire=open(File,'a')
    Ecrire.write("FileVCF1 : "+Nom1+" Nb Locus : "+str(len(VCF1['InfoSnp'].keys())) +"\n")
    Ecrire.write("FileVCF2 : "+Nom2+ " Nb Locus : "+str(len(VCF2['InfoSnp'].keys())) +  "\n")
    Ecrire.write("Nb Locus Commun : "+ str(len(ListeLocus)))
...
def LaunchCommande():
     ...
     if len(ListeFichierVCF) == 0:
        sys.exit(Commande)
     FichierListeIndSouhaite=GetUnArgument(sys.argv, '-li')
     if len(FichierListeIndSouhaite)==0 :
        ListeIndSouhaite=None
     else :
        ListeIndSouhaite=GetEntete(FichierListeIndSouhaite[0])
     ...
     if len(ListeFichierVCF)==1 and Fichier50K==None :
        SaveInfo=False
     DicoVCF=GetDataVCF(ListeFichierVCF,BaliseBCF,Entete, SaveInfo, ListeIndSouhaite=ListeIndSouhaite)
     if Fichier50K!= None :
        DicoSnpPuce=GetInfoVCF(Fichier50K, Fichier50K+".brut.ind.stat",Fichier50K+".brut.locus.stat", ListeIndSouhaite=ListeIndSouhaite)
        CompareVCFWithPuce(DicoVCF,  DicoSnpPuce)
     Cmt=0
     if len(ListeFichierVCF)>1 :
