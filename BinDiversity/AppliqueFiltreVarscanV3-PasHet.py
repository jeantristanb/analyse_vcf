...
def GetInfoIndCSVFiltre(StrInfoInd, MinCouv, MaxCouv, PercPEError, PercRed,PValue ,IsHomozygoteFiltreX, BHom, BHet, ListeAlt):
    A1=0
    A2=0
...
###def IsHomozygoteFiltreV6 (NbRef, NbAlt, NbPEI, PercAlt, PValue,ErrorRate, BHom, BHet):
        ...
        if AlleleStatut==-1 :
           ...
           ## si on ne peut pas definir de nouvel allele et que l'allele est quand meme deifini
           if ListeAlt[0]=='.' :
              A1=0
              A2=0
              NbAllele=0
...
def GetInfoAllIndividu(SplitLigne, MinCouv, MaxCouv,PercPEError,MaxPercRed,Pvalue ,FoncFiltre, PositionAvecIDHet, BaliseHetID, BHom, BHet) :
    ...
    for Cmt in range(9, len(SplitLigne)) :
        InfoInd=GetInfoIndCSVFiltre(SplitLigne[Cmt], MinCouv, MaxCouv,PercPEError,MaxPercRed,Pvalue ,FoncFiltre, BHom, BHet, ListeAllele)
