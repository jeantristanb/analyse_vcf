...
def GetInfoIndCSVFiltre(StrInfoInd, MinCouv, MaxCouv, PercPEError, PercRed,PValue ,IsHomozygoteFiltreX, BHom, BHet):
    A1=0
    A2=0
...
##   
def IsBonLocus(FreqInd, MeanCouvLocus,NbPerrorHom, NbPeHom, ListeAlt, Ref,MaxCouvMean ,MinNbHomAlt, PercPEError,NbLocusMauvaisDupIDD, NbAlt, PValue, TypePoly, PercNaMax, DistributionErreur, BaliseKs, BaliseHetRed1Loc) :
...
#       return [False]
   ...
   if NbC==0:
        return [False]
   if (FreqInd[1] <MinNbHomAlt or  FreqInd[3]<MinNbHomAlt) and FreqInd[0]>0:
        return [False]
...
def GetInfoAllIndividu(SplitLigne, MinCouv, MaxCouv,PercPEError,MaxPercRed,Pvalue ,FoncFiltre, PositionAvecIDHet, BaliseHetID, BHom, BHet) :
    Chro=SplitLigne[0]
    Pos=SplitLigne[1]
    Ref=SplitLigne[3]
    Alt=SplitLigne[4]
    ...
    for Cmt in range(9, len(SplitLigne)) :
        InfoInd=GetInfoIndCSVFiltre(SplitLigne[Cmt], MinCouv, MaxCouv,PercPEError,MaxPercRed,Pvalue ,FoncFiltre, BHom, BHet)
...
def CheckInsertionDeletion(ListePos,PosCourante, Dist) :
     ...
     for Pos in ListePos:
         ...
         if abs(DiffPos)<Dist :
            return False
         if DiffPos>Dist :
            #raw_input('del '+ str(DiffPos)+ " "+str(Dist))
            ListePos.remove(Pos)
         elif DiffPos<Dist :
            return True
         #   return True
         #elif abs(DiffPos)>Dist :
         #   raw_input('del '+ str(DiffPos)+ " "+str(Dist))
         #   ListePos.remove(Pos)
     return True
...
def GetTransforIndividu(ListeInfoInd, PositionAvecIDHet, EcrireStat, Ecrire, PosInt, FonctionFiltreBonLocus, MaxCouvMean ,MinNbHomAlt, PercPEError, NbAltParam,  Pvalue, TypePoly, PercNaMax,  BaliseKs, StatParInd, PrintAllLocus, BaliseHetID, BaliseHetRed1Loc):
           ...
           for Cmt in range(len(ListeInfoInd[1])):
               ...
               if BaliseHetID and int(InfoInd[4])==2:
                  BaliseID=CheckInsertionDeletion(PositionAvecIDHet[Cmt], PosInt, 250)
                  if BaliseID==False :
           ...
           BonLocus=FonctionFiltreBonLocus(FreqInd,MeanCouvLocus,NbPEErreurHom,  NbPEITotHom, ListeAlt[3], Ref,MaxCouvMean ,MinNbHomAlt, PercPEError,NbLocusMauvaisDupID ,NbAltParam,  Pvalue, TypePoly, PercNaMax, DistributionErreur, BaliseKs, BaliseHetRed1Loc)
           ## On recalcu
           if BonLocus[0] :
    ...
    for Ligne in Lire :
       ## On verifie qu'il ne s'agit pas d'une entete
        if Ligne[0]!="#" and len(Ligne)>5 : 
           ...
           ListeInfo1Locus=GetInfoAllIndividu(SplitLigne, MinCouv, MaxCouv,PercPEError,MaxPercRed,Pvalue ,FoncFiltre, PositionAvecIDHet, BaliseHetID, BHom,BHet)
...
def Launch():
    ListeArg=["-f", "-i","-o","-os", "-e", "-snf", "-cmin", "-prm", "-pv", "-pa", "-cmax", "-cmaxmean", "-nballmax", "-nbindalt", "-percnamax", "-osi", "-tks", "-hetdlid", "-hetinddup"]
