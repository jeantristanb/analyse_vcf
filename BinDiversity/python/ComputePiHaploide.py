...
def TajimaDWithMissingData(Liste1,Liste2,PreComputea1,PreComputea2,MaxN, MinFreqNa, MinFreqS) :
    SumDifTheta=0
    SumThetaPi=0
    SumThetaW=0
    #MaxN2=float(MaxN)
...
def nCr(n,r):
    fact = math.factorial
    return fact(n) / float(fact(r)) / float(fact(n-r))
...
def GetPiCresko(n1,n2):
    ...
    if n1<1  or n2 < 1 :
       ...
       return (n1+n2-1)/float(sum(range(1,int(n1+n2))))
    return 1- (nCr(n1,2)+nCr(n2,2))/float(nCr(n1+n2,2) )

def GetPiFreq(n1,n2) :
       N=n1 + n2
       Freq=n1/float(N)
       return N/(float(N)-1.0)*2.0*(Freq*(1-Freq))
...
def ComputeAllPiOneLocus(a, n, N, MinNbInd, MinNbPosition=None) :
   ...
   if n>=MinNbInd :
      PiGre=GetPiCresko(a, n-a)
      PiFreq=GetPiFreq(a, n-a)
   else :
      PiGre="NA"
      PiFreq="NA"
   return (PiGre, PiFreq, a, n)
